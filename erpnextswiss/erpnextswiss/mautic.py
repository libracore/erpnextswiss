...
class MauticAPI:
    ...
    def load_credentials(self):
        ...
        try:
            ...
            self.base_url = settings.mautic_url.rstrip('/')
...
def sync_customer_to_mautic(customer_name):
    ...
    company_data = {
        ...
        "companyphone": customer.mobile_no or customer.phone_no or "",
        "companyaddress1": customer.customer_primary_address or "",
        "companycity": customer.get("city") or "",
        "companystate": customer.get("state") or "",
        "companycountry": customer.get("country") or "",
        "companywebsite": customer.website or "",
    }
    
    # Prüfe, ob Kunde bereits in Mautic existiert
    mautic_id = customer.get("mautic_company_id")
    ...
    if mautic_id:
        ...
        if result and 'company' in result:
            customer.db_set('mautic_company_id', result['company']['id'])
...
def sync_contact_to_mautic(contact_name):
    ...
    mautic_id = contact.get("mautic_contact_id")
    ...
    if mautic_id:
        ...
        if result and 'contact' in result:
            ...
            contact.db_set('mautic_contact_id', new_contact_id)
