...
def generate_payment_file(payments):
    ...
    content += make_line("        <NbOfTxs>" + len(payments) + "</NbOfTxs>")
    # total amount of all transactions ( e.g. 15850.00 )  (sum of all amounts)
    content += make_line("        <CtrlSum>" + get_total_amount(payments) + "</CtrlSum>")
    ...
    for payment in payments:
        ...
        payment_content += make_line("      <ReqdExctnDt>" + 
            payment_record.posting_date + "</ReqdExctnDt>")
        ...
        payment_content += make_line("          <InstdAmt Ccy=\"" + 
            payment_record.paid_from_account_currency + "\">" + 
            payment_record.paid_amount + " </InstdAmt>")
        payment_content += make_line("        </Amt>")
        # creditor account
        ...
        if payment_record.transaction_type == "ESR":
            ...
            if payment_record.esr_participant_number:
                ...
                content += add_invalid_remark( payment + ": " + __("no ESR participation number found") )
                continue
            payment_content += make_line("            </Othr>")
            payment_content += make_line("          </Id>")
            payment_content += make_line("        </CdtrAcct>")
            ...
            if payment_record.esr_reference:
                ...
                content += add_invalid_remark( payment + ": " + __("no ESR reference found") )
                continue    
            payment_content += make_line("            </CdtrRefInf>")
            payment_content += make_line("          </Strd>")
            payment_content += make_line("        </RmtInf>")
            ...
            if payment_reference.iban:
                ...
                content += add_invalid_remark( payment + ": " + __("no IBAN found") )
                continue
            payment_content += make_line("          </Id>")
            payment_content += make_line("        </CdtrAcct>")
            # creditor agent
            ...
            if payment_reference.bic:
                ...
                content += add_invalid_remark( payment + ": " + __("no BIC found") )
                continue
            payment_content += make_line("          </FinInstnId>")
            payment_content += make_line("        </CdtrAgt>")      
            ...
            if supplier_address == None
                ...
                content += add_invalid_remark( payment + ": " + __("no address found") )
                continue
            ...
            # postal code
            payment_content += make_line("            <PstCd>" +
                supplier_address.postal_code + "</PstCd>"
            # town name
