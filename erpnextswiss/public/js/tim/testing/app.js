...
function initTimApi() {	
    ...
    frappe.db.get_doc("Worldline TIM", "Worldline TIM").then(function(details){
        ...
        settings.connectionIPString = details.test_connectionipstring;
        settings.connectionIPPort = details.test_connectionipport;

        // The integrator id to identify the integrator
        settings.integratorId = details.test_integratorid;

        // Deactivate fetch brands and auto commit
        settings.fetchBrands = false;
        settings.autoCommit = parseInt(details.test_autocommit) == 1 ? true:false;
