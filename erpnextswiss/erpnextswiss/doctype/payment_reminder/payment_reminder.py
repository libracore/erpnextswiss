...
def create_payment_reminders():
    ...
    sql_query = ("""SELECT `customer` 
            ...
            AND `exclude_from_payment_reminder_until` >= CURDATE()
            GROUP BY `customer`;""")
    ...
    if customers:
        for customer in customers:
            sql_query = ("""SELECT `name`, `due_date`, `payment_reminder_level`, `grand_total` 
                    FROM `tabSales Invoice` 
                    WHERE `outstanding_amount` > 0 AND `customer` = '{'}
                    AND `exclude_from_payment_reminder_until` >= CURDATE();""".format(customer.customer))
            ...
            if open_invoices:
                ...
                new_reminder = frappe.get_doc({
                    ...
                    "title": "{0} {1}-{2}-{3}".format(customer, now.year, now.month, now.day),
