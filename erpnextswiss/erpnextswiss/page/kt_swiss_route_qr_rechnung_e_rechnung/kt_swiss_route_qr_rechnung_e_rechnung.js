...
frappe.pages["kt-swiss-route-qr-rechnung-e-rechnung"].on_page_show = function() {
    const allowed = (frappe.boot.workspaces?.pages || []).some(item => item.name === "QR-Rechnung & E-Rechnung");
    if (!allowed) {
        frappe.throw(__("Not permitted"));
        return;
    }
    setTimeout(function() { frappe.set_route(frappe.router.slug("QR-Rechnung & E-Rechnung")); }, 0);
};
